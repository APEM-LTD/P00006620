...
Flows <- Flows_master %>%
  ...
  dplyr::mutate(wetted_width_m = (max_width_m - min_width_m)) %>% #diff between max and min
  ...
  dplyr::mutate(discharge = mean_vel_msec * mean_depth_cm * wetted_width_m) %>% #calculate discharges by multiplying mean velocity by mean depth by channel width per specific transect
  ...
  dplyr::mutate(vegetation = ifelse(substrate_type %like% "Veg|veg|ranunculus|vegetation|Vegetation|macrophyte|watercress|berula|sparganiumSparganium|glyceria|Glyceria", 1, 0)) %>%
...
## testing if possible to calculate full wetted width, without excluding first channel section... Works, but assumes near bank is wetted, when depth != 0 at min(dist_from_bank)
#  dplyr::mutate(min_width_wetted = case_when(
#    (dist_from_bank == min(dist_from_bank) & depth != 0) ~ 0, 
#    TRUE ~ dist_from_bank)) %>%
#  dplyr::filter(depth != 0) %>%
#  dplyr::mutate(min_width_wetted = min(min_width_wetted, na.rm = TRUE)) %>%
#  dplyr::mutate(max_width_m = max(dist_from_bank, na.rm = TRUE)) %>% #diff between max and min
#  dplyr::mutate(chan_width_m = (max_width_m - min_width_wetted)) %>% #diff


## summary data
Flows_Summary <- Flows %>%
  dplyr::select(-depth, -dist_from_bank, -velocity, -substrate_type) %>%
  group_by(site_id, site_name, date, year, month, season) %>%
  dplyr::filter(!duplicated(min_width_m))
